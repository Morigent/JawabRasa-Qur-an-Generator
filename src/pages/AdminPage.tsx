...
function ConsultantCard({
  ...
  return (
    <div className={clsx('bg-surface', 'vellum-texture', 'gold-border', 'rounded-xl', 'shadow-[0_10px_30px_rgba(6,78,59,0.05)]', 'p-6', 'flex', 'flex-col', 'gap-4', 'border', 'border-outline-variant/10', 'transition-all', 'duration-300', 'hover:shadow-[0_16px_40px_rgba(6,78,59,0.1)]')}>
      ...
      {c.bio && (
        <div>
          <p className={`text-sm text-on-surface-variant italic ${expanded ? '' : 'line-clamp-2'}`}>
            &ldquo;{c.bio}&rdquo;
          </p>
          {c.bio.length > 80 && (
            <button
      ...
      {c.verification_status !== 'pending' && (
        <div className={`rounded-lg px-3 py-2 text-center text-xs font-ui-label-caps ${VERIF_STYLES[c.verification_status]}`}>
          ...
          {activeSection === 'consultants' && (
            <section className="reveal-animation">
              <div className={clsx('flex', 'items-center', 'justify-between', 'mb-6')}>
                ...
                {pendingCount > 0 && (
                  <span className={clsx('bg-secondary-container', 'px-4', 'py-1', 'rounded-full', 'font-ui-label-caps', 'text-ui-label-caps', 'text-on-secondary-container')}>
                          ...
                          : users.map((u) => (
                            ...
                            >
                              ...
                              <td className={clsx('px-6', 'pr-8', 'py-5', 'text-right')}>
                                <button className={clsx('material-symbols-outlined', 'text-on-surface-variant', 'hover:text-primary', 'transition-colors')}>
                                  more_vert
                                </button>
                              </td>
                            </tr>
